...
#define CONFIG_SCO_OVER_HCI     1 /* Enabled SCO OVER HCI */
...
#define HCI_VENDOR_READ_RTK_KEY_ID 0xfc61
...
//signature: rtbtcore
const uint8_t RTK_EPATCH_SIGNATURE_V2[8]={0x52,0x54,0x42,0x54,0x43,0x6F,0x72,0x65};
...
static void line_process(char *buf, unsigned short *offset, int *t)
{
    ...
    if(buf[0] == '\0' || buf[0] == '#' || buf[0] == '[')
        return;
    if(alt_size > max_extra_extry_size-4)
    {
        RTKBT_WARN("Extra Config file is too large");
        return;
    }
    if(extra_entry_inx == NULL)
        extra_entry_inx = extra_extry;
    RTKBT_INFO("line_process:%s", buf);
    while((ptr = strsep(&head, ", \t")) != NULL)
    {
        ...
        argv[argc++] = ptr;
        if(argc >= 32) {
            RTKBT_WARN("Config item is too long");
            break;
        }
    }

    if(argc <4) {
        RTKBT_ERR("Invalid Config item, ignore");
        return;
    }

    offset[(*t)] = (unsigned short)((simple_strtoul(argv[0], NULL, 16)) | (simple_strtoul(argv[1], NULL, 16) << 8));
    RTKBT_INFO("Extra Config offset %04x", offset[(*t)]);
    extra_entry_inx->offset = offset[(*t)];
    (*t)++;
    len = (unsigned char)simple_strtoul(argv[2], NULL, 16);
    if(len != (unsigned char)(argc - 3)) {
        RTKBT_ERR("Extra Config item len %d is not match, we assume the actual len is %d", len, (argc-3));
        len = argc -3;
    }
    extra_entry_inx->entry_len = len;

    alt_size += len + sizeof(struct rtk_bt_vendor_config_entry);
    if(alt_size > max_extra_extry_size)
    {
        RTKBT_WARN("Extra Config file is too large");
    ...
    for(i = 0; i < len; i++)
    {
        extra_entry_inx->entry_data[i] = (uint8_t)simple_strtoul(argv[3+i], NULL, 16);
        RTKBT_INFO("data[%d]:%02x", i, extra_entry_inx->entry_data[i]);
    }
...
static void parse_extra_config(const char *path, patch_info *patch_entry, unsigned short *offset, int *t)
{
    ...
    if (IS_ERR(fp)){
        RTKBT_INFO("No Extra Config file\n");
        return;
    }
    oldfs = get_fs(); set_fs(KERNEL_DS);
    fp->f_op->llseek(fp, 0, 0);
    ret = vfs_read(fp, buf, 1024, &fp->f_pos);
    set_fs(oldfs);
    if(ret < 0) {
        RTKBT_ERR("Couldn't read %s, err:%d", path, ret);
        filp_close(fp, NULL);
        return;
    }
    else if(ret == 0) {
        RTKBT_ERR("%s is empty", path);
        filp_close(fp, NULL);
        return;
    }

    if(ret > 1022) {
        RTKBT_ERR("Extra config file is too big");
        filp_close(fp, NULL);
        return;
    }
    buf[ret++] = '\n';
...

    {
        ...
        {
            RTKBT_WARN("Extra config file not set for %s, ignore", patch_entry->config_name);
            return;
        }
    ...
    while(ptr->offset)
    {
        if(ptr->offset == offset)
        {
            if(offset != patch_entry->mac_offset)
            {
                ...
                res = ptr->entry_len;
                RTKBT_INFO("Get Extra offset:%04x, val:", offset);
                for(i = 0; i < ptr->entry_len; i++)
                    RTKBT_INFO("%02x", ptr->entry_data[i]);
            }
            break;
        }
