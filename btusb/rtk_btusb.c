...
#define RTKBT_RELEASE_NAME "20220525_BT_ANDROID_6.0"
...
{ 0x0BDA, 0xA82A, 0x8822, 0, 0, "mp_rtl8822e_fw", "rtl8822e_fw", "rtl8822e_config", NULL, 0 ,CONFIG_MAC_OFFSET_GEN_4PLUS, MAX_PATCH_SIZE_145K}, /* RTL8822EU */
{ 0x0BDA, 0xA82B, 0x8822, 0, 0, "mp_rtl8822e_fw", "rtl8822e_fw", "rtl8822e_config", NULL, 0 ,CONFIG_MAC_OFFSET_GEN_4PLUS, MAX_PATCH_SIZE_145K}, /* RTL8822EU */
{ 0x0BDA, 0xA822, 0x8822, 0, 0, "mp_rtl8822e_fw", "rtl8822e_fw", "rtl8822e_config", NULL, 0 ,CONFIG_MAC_OFFSET_GEN_4PLUS, MAX_PATCH_SIZE_145K}, /* RTL8822EU */
{ 0x0BDA, 0xE822, 0x8822, 0, 0, "mp_rtl8822e_fw", "rtl8822e_fw", "rtl8822e_config", NULL, 0 ,CONFIG_MAC_OFFSET_GEN_4PLUS, MAX_PATCH_SIZE_145K}, /* RTL8822EU */
/* todo: RTL8703CU */
...
{ 0x0BDA, 0xB852, 0x8852, 0, 0, "mp_rtl8852au_fw", "rtl8852au_fw", "rtl8852au_config", NULL, 0 ,CONFIG_MAC_OFFSET_GEN_4PLUS, MAX_PATCH_SIZE_69_2K}, /*RTL8852B */
{ 0x0BDA, 0xA85B, 0x8852, 0, 0, "mp_rtl8852bu_fw", "rtl8852bu_fw", "rtl8852bu_config", NULL, 0 ,CONFIG_MAC_OFFSET_GEN_4PLUS, MAX_PATCH_SIZE_69_2K}, /*RTL8852B */
{ 0x0BDA, 0xB85B, 0x8852, 0, 0, "mp_rtl8852bu_fw", "rtl8852bu_fw", "rtl8852bu_config", NULL, 0 ,CONFIG_MAC_OFFSET_GEN_4PLUS, MAX_PATCH_SIZE_69_2K}, /*RTL8852B */
{ 0x0BDA, 0xC85A, 0x8852, 0, 0, "mp_rtl8852cu_fw", "rtl8852cu_fw", "rtl8852cu_config", NULL, 0 ,CONFIG_MAC_OFFSET_GEN_4PLUS, MAX_PATCH_SIZE_69_2K}, /*RTL8852C */
...
{ 0x0BDA, 0xA85C, 0x8852, 0, 0, "mp_rtl8852bpu_fw", "rtl8852bpu_fw", "rtl8852bpu_config", NULL, 0 ,CONFIG_MAC_OFFSET_GEN_4PLUS, MAX_PATCH_SIZE_69_2K}, /*RTL8852BP */
{ 0x0BDA, 0xA850, 0x8852, 0, 0, "mp_rtl8852bpu_fw", "rtl8852bpu_fw", "rtl8852bpu_config", NULL, 0 ,CONFIG_MAC_OFFSET_GEN_4PLUS, MAX_PATCH_SIZE_69_2K}, /*RTL8852BPE */
        ...
        loff_t *pos_p)
{
    ...
    while (count) {
        ...
        if (ret < 0) {
            RTKBT_ERR("%s: wait event is signaled %lu", __func__, (unsigned long)ret);
            break;
        }
...
int get_key_id(firmware_info* fw_info)
{
    int ret_val;
    uint8_t key_id;
    uint8_t status;
    uint8_t data[5] = {0x10, 0xa4, 0xad, 0x00, 0xb0};
    if (!fw_info)
        return -ENODEV;

    fw_info->cmd_hdr->opcode = cpu_to_le16(HCI_VENDOR_READ_RTK_KEY_ID);
    fw_info->cmd_hdr->plen = 5;
    memcpy(fw_info->req_para, data, 5);
    fw_info->pkt_len = CMD_HDR_LEN + 5;
    ...
    status = *(fw_info->rsp_para);
    key_id = *(fw_info->rsp_para + 1);

    RTKBT_INFO("%s: status %d, eversion %d", __func__, status, key_id);

    if (status){
        fw_info->patch_entry->key_id = 0;
        ret_val = -1;
    }
    else
        fw_info->patch_entry->key_id = key_id;
...
void rtk_update_altsettings(patch_info *patch_entry, const unsigned char* org_config_buf, int org_config_len, unsigned char ** new_config_buf_ptr, int *new_config_len_ptr)
{
    ...
    max_extra_extry_size = sizeof(config_buf) - org_config_len;
    if((extra_extry = (struct rtk_bt_vendor_config_entry *)kzalloc(max_extra_extry_size, GFP_KERNEL)) == NULL)
        RTKBT_ERR("kzalloc buffer for extra_extry failed");
...
uint8_t get_fw_parsing_rule(uint8_t *epatch_buf)
{
    uint8_t opcode;
    uint8_t len;
    uint8_t data = 1;
    ...
    do {
        ...
        if (opcode == 0x01)
        {
            ...
            {
                data = *(epatch_buf - 2);
                RTKBT_INFO("rtk_get_fw_parsing_rule: opcode %d, len %d, data %d", opcode, len, data);
                break;
            }
            else
            {
                RTKBT_INFO("rtk_get_fw_parsing_rule: invalid len %d", len);
            }
        }
        epatch_buf -= len + 2;
    } while (*epatch_buf != 0xFF);

    return data;
}


uint8_t insert_fw_patch_fragment_to_linklist(struct rtk_epatch_fragment *fragment,
        struct rtk_epatch_fragment_linklist **header)
{
    ...
    tmp = (struct rtk_epatch_fragment_linklist *)vmalloc(sizeof(*p));
    //RTKBT_INFO("insert_fw_patch_fragment_to_linklist ");
    if(!tmp)
    {
        RTKBT_ERR("insert_fw_patch_fragment_to_linklist: failed to allocate mem for patch entry");
        return -1;
    }
 ...
 uint32_t get_fw_patch_link_list(uint8_t * epatch_buf,
                    struct rtk_epatch_fragment_linklist **linklist, uint16_t chip_id, uint8_t key_id)
  {
      uint8_t res;
      uint16_t i,j;
      ...
      patch = (struct rtk_epatch_v2 *)epatch_buf;

      patch->number_of_section = le16_to_cpu(patch->number_of_section);

      RTKBT_INFO("rtk_get_fw_patch_link_list: fw_ver 0x%08x,  fw_ver_sub 0x%08x, number_of_section %d",
     ...
     //Traversal section
      for(i = 0; i < patch->number_of_section; i++)
      {
          ...
          RTKBT_INFO("rtk_get_fw_patch_link_list: opcode: %d,  length:%d, number_of_fragment: %d",
          ...
          for(j = 0; j < section->number_of_fragment; j++){
              if(section->opcode != FW_OTA_FLAG){
                  ...
                  RTKBT_INFO("rtk_get_fw_patch_link_list: chip_id: %d,  priority:%d, length:  0x%x",
              ...
              if(to_add){
                  res = insert_fw_patch_fragment_to_linklist(fragment, &link_header);
                  if(res)
                     goto free_linklist;
                  to_add = false;
                  ...
                  /*{
                      data = fragment->data;
                      RTKBT_INFO("fragment->data  %02x %02x %02x %02x %02x %02x %02x %02x",
      ...
      if( (key_id != 0x00) && (key_id != 0xff) && (sec_sig_cnt == 0)){
         ...
         for(i = 0; i < patch->number_of_section; i++)
         {
             ...
             section->length = le32_to_cpu(section->length);
             section->number_of_fragment = le16_to_cpu(section->number_of_fragment);
             RTKBT_INFO("rtk_get_fw_patch_link_list 2: opcode: %d,  length:%d, number_of_fragment: %d",
                section->opcode, section->length, section->number_of_fragment);
...
uint32_t get_v2_final_fw(uint8_t * epatch_buf, patch_info * patch_entry, uint8_t ** buff, int config_len)
{
    uint8_t *p, *data;
    uint32_t fw_patch_len = 0;
    uint32_t fw_version, svn_version, coex_version;
    uint16_t chip_id = patch_entry->eversion + 1;
    uint8_t  key_id = patch_entry->key_id;
    uint32_t  total_len = 0;
    struct rtk_epatch_fragment_linklist *fw_patch_link = NULL;
    struct rtk_epatch_fragment_linklist * tmp;

    fw_patch_len = get_fw_patch_link_list(epatch_buf, &fw_patch_link, chip_id, key_id);

    total_len = fw_patch_len + config_len;

    RTKBT_INFO("fw_patch_len = 0x%x, total_len = 0x%x", fw_patch_len, total_len);

    if (!(*buff = kzalloc(total_len, GFP_KERNEL)))
    {
    ...
    {
        RTKBT_INFO("get_v2_final_fw buff %p ", *buff);
        p = *buff;
        tmp = fw_patch_link;
        //fw_patch_len = 0;
        RTKBT_INFO("copy patch fragment to total_buf");
        while(tmp)
        {
            if(tmp->fragment)
            {
                RTKBT_INFO("fragment->priority= %d, fragment->length = 0x%x",
...
*/
    memcpy(&fw_version, *buff + fw_patch_len - 4, 4);
    memcpy(&svn_version, *buff + fw_patch_len - 8, 4);
    memcpy(&coex_version, *buff + fw_patch_len - 12, 4);
    patch_entry->lmp_sub = (uint16_t)fw_version;

    RTKBT_INFO("BTCOEX:20%06d-%04x svn_version:%d fw_version:%04x  Cut:%d ",
    ((coex_version >> 16) & 0x7ff) + ((coex_version >> 27) * 10000),
    (coex_version & 0xffff), svn_version, fw_version, chip_id);
    ...
    RTKBT_INFO("get_v2_final_fw total_len %d", total_len);
    return total_len;
}
...
//        read_localversion(fw_info);
        ...
        parsing_rule = get_fw_parsing_rule(epatch_buf + fw_len - 5);
        if (memcmp(epatch_buf + buf_len - config_len - 4 , EXTENSION_SECTION_SIGNATURE, 4)) {
            ...
            if (lmp_version != project_id[*(patch_lmp.data)]) {
                ...
                if(parsing_rule == 1){
                    RTKBT_INFO("%s: fw patch parsing_rule v1", __func__);
                    if (memcmp(epatch_buf, RTK_EPATCH_SIGNATURE, 8)) {
                        RTKBT_ERR("%s: Check signature error 1", __func__);
                        need_download_fw = 0;
                    } else {
                    ...
                    if (memcmp(epatch_buf, RTK_EPATCH_SIGNATURE_V2, 8)) {
                        RTKBT_ERR("%s: Check signature error 2", __func__);
                        need_download_fw = 0;
                    } else {
                        ret_val = get_key_id(fw_info);
                        if (ret_val < 0) {
                            RTKBT_ERR("%s: Failed to get keyid, errno %d", __func__, ret_val);
                            goto fw_fail;
                        }
                        RTKBT_INFO("%s: Get keyid =%d", __func__, patch_entry->key_id);
                        buf_len = get_v2_final_fw(epatch_buf, patch_entry, &buf, config_len);
                        RTKBT_INFO("%s: buf %p ", __func__, buf);
                        if(buf_len > 0 ){
                            if(config_len)
                               memcpy(&buf[buf_len - config_len], config_file_buf, config_len);
                        }else{
    ...
    if (lmp_version != ROM_LMP_8723a) {
        ...
        if (memcmp(epatch_buf + buf_len - 4 , EXTENSION_SECTION_SIGNATURE, 4)) {
            ...
            if (lmp_version != project_id[*(patch_lmp.data)]) {
                ...
                RTKBT_INFO("%s: Default lmp_version 0x%04x, project_id[%d] 0x%04x "
                        "-> match", __func__, lmp_version, *(patch_lmp.data), project_id[*(patch_lmp.data)]);
                parsing_rule = get_fw_parsing_rule(epatch_buf + buf_len - 5);
                RTKBT_INFO("%s: parsing_rule %d", __func__, parsing_rule);
                if(parsing_rule == 1){
                    if (memcmp(epatch_buf, RTK_EPATCH_SIGNATURE, 8)) {
                       RTKBT_ERR("%s: Check signature error 3", __func__);
                    }
                    else {
                       epatch_info = (struct rtk_epatch*)epatch_buf;
                       patch_entry->lmp_sub = (uint16_t)epatch_info->fw_version;
                  ...
                  if (memcmp(epatch_buf, RTK_EPATCH_SIGNATURE_V2, 8)) {
                       RTKBT_ERR("%s: Check signature error 5", __func__);
                  }
                  else {
                      epatch_info = (struct rtk_epatch*)epatch_buf;
                      patch_entry->lmp_sub = (uint16_t)epatch_info->fw_version;
    ...
    if (memcmp(epatch_buf + buf_len - config_len - 4 , EXTENSION_SECTION_SIGNATURE, 4)) {
        ...
        if (lmp_version != project_id[*(patch_lmp.data)]) {
            ...
            if (patch_lmp.data)
                kfree(patch_lmp.data);
            if (memcmp(epatch_buf, RTK_EPATCH_SIGNATURE, 8)) {
                RTKBT_ERR("%s: Check signature error 4", __func__);
                need_download_fw = 0;
            } else {
                epatch_info = (struct rtk_epatch*)epatch_buf;
                patch_entry->lmp_sub = (uint16_t)epatch_info->fw_version;
...
static RTK_sco_card_t* btusb_snd_init(struct usb_interface *intf, const struct usb_device_id *id, struct btusb_data *data)
{
    ...
    err = snd_card_create(-1,
     RTK_SCO_ID, THIS_MODULE,
